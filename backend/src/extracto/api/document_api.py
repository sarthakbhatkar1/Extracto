...
async def list_of_documents():
    ...
    try:
        ...
        # raise Exception(f'Exception in listing of documents: {e}')
...
async def upload_document(
    ...
    try:
        ...
        # raise Exception(f'Exception in uploading document: {e}')
    return json_response.dict()
...
async def download_document(documentId: str):
    json_response = JsonResponse()
    try:
        response = await DocumentService().download(documentId=documentId)
        print(f"Successfully retrieved the document with documentId: {documentId}.")
        json_response.result = response
        json_response.success = True
    except Exception as e:
